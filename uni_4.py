...
class UNET(nn.Module):
  ...
  def forward(self,x):
    ...
    d10 = self.Up5(x10)
    d10 = torch.cat((x4,d10),dim=1)
    d10 = self.Up_conv5(d10)
    
    d9 = self.Up4(d10)
    d9 = torch.cat((x3,d9),dim=1)
    d9 = self.Up_conv4(d9)
    ...
    d6 = self.Conv_1x1(d7)
    d6 = self.BatchNorm(d6)

    d0 = torch.cat((d1,d6),dim=1)
    d0 = torch.sigmoid(d0)
...
def main():
 ...
 # )
  ...
  # 设置优化器
  optimzer = torch.optim.SGD(unet.parameters(), lr=0.05,momentum=0.9,weight_decay=0.001)
  ...
  for epoch in range(1500+1):
    ...
    #     loss = loss_func(out, label)  # 计算误差
          ...
          loss_1= 0.5*loss_func_1(out[0][0],batch_label)+L1_lambda*L1_regularization_loss
          loss_2= 0.5*loss_func_2(out[0][1],batch_x/255)
          loss=loss_1+loss_2
          if epoch%5==0:
            predict=out>0.5
            acc+=float(torch.sum(predict[0][0]==batch_label[0][0]))/(512*512)
            if epoch%150==0 or epoch == 20 or epoch == 50 or epoch == 100:
              predict=(predict).int()
              os.makedirs('./'+str(epoch)+'trainresult_4', exist_ok=True)
              train_img = Image.fromarray(predict[0][0].detach().cpu().numpy().reshape(1,512,512)[0].astype('uint8')*255)
              train_img.save('./'+str(epoch)+'trainresult_4/'+str(step)+'.png')
    ...
    if epoch%5==0:
      ...
      #     out=unet.forward(x)
            ...
            if epoch%150==0 or epoch == 20 or epoch == 50 or epoch == 100:
              os.makedirs('./'+str(epoch)+'testresult_4', exist_ok=True)
              ...
              test_img.save('./'+str(epoch)+'testresult_4/'+str(test_step)+'.png')
      #     loss = loss_func(out, label)  # 计算误差
    ...
    if epoch%5==0:
      ...
      torch.save(unet,'./unet_4.pkl')
  #model=torch.load('./unet_4.pkl') 
  np.save('./loss_4.npy',loss_im)
  np.save('./step_4.npy',step_im)
  np.save('./acc_4.npy',acc_im)
  np.save('./teacc_4.npy',te_acc_im)
  loss_im=np.load('./loss_4.npy')
  step_im=np.load('./step_4.npy')
  ...
  plt.savefig('./test_accuracy_4.png')
  plt.show()
  # print(myNet(x).data)
