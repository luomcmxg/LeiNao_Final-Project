  ...
  for epoch in range(1500+1):
    ...
    #     loss = loss_func(out, label)  # 计算误差
          ...
          if epoch%5==0:
            ...
            if epoch%100==0:
              predict=(predict).int()
              os.makedirs('./'+str(epoch)+'trainresult_2', exist_ok=True)
              train_img = Image.fromarray(predict.detach().cpu().numpy().reshape(1,512,512)[0].astype('uint8')*255)
              train_img.save('./'+str(epoch)+'trainresult_2/'+str(step)+'.png')
    ...
    if epoch%5==0:
      ...
      torch.save(unet,'./unet_2.pkl')
  #model=torch.load('./unet_2.pkl') 
  np.save('./loss_2.npy',loss_im)
  np.save('./step_2.npy',step_im)
  loss_im=np.load('./loss_2.npy')
  step_im=np.load('./step_2.npy')
