...
class UNET(nn.Module):
  ...
  def forward(self,x,bd2=None,bd3=None,bd4=None,bd5=None):
    # encoding path
    x1 = self.Conv1(x)
    if (bd2==None):
      bd2 = x1
    x1c = torch.cat((x1,bd2),dim=1)

    x2 = self.Maxpool(x1c)
    x2 = self.Conv2(x2)
    if (bd3==None):
      bd3 = x2
    x2c = torch.cat((x2,bd3),dim=1)
    
    x3 = self.Maxpool(x2c)
    x3 = self.Conv3(x3)
    if (bd4==None):
      bd4 = x3
    x3c = torch.cat((x3,bd4),dim=1)

    x4 = self.Maxpool(x3c)
    x4 = self.Conv4(x4)
    if (bd5==None):
      bd5 = x4
    x4c = torch.cat((x4,bd5),dim=1)
...
def main():
 ...
 # )
  ...
  bd2, bd3, bd4, bd5 = None, None, None, None
  for epoch in range(500+1):
    ...
    #     out=unet.forward(x)
          ...
          out,bd2,bd3,bd4,bd5=unet(batch_x,bd2,bd3,bd4,bd5)
          bd2 = repackage_hidden(bd2)
          bd3 = repackage_hidden(bd3)
          bd4 = repackage_hidden(bd4)
          bd5 = repackage_hidden(bd5)
    #     loss = loss_func(out, label)  # 计算误差
          ...
          if epoch%5==0:
            predict=out>0.5
            acc+=(torch.sum(predict[0][0].float()==batch_label[0][0]))/(512*512)
            if epoch%50==0:
              predict=(predict).int()
    ...
    if epoch%5==0:
      ...
      #     out=unet.forward(x)
            ...
            test_out,_,_,_,_=unet(batch_test_x)
            if epoch%50==0:
